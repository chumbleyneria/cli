...
func TestMinutesWordReplace_WordsNotFound(t *testing.T) {
	...
	if exitErr.Detail == nil || exitErr.Detail.Type != "words_not_found" {
		t.Fatalf("expected words_not_found detail, got %#v", exitErr.Detail)
	}
	if !strings.Contains(exitErr.Detail.Message, minutesWordReplaceTestToken) {
		t.Errorf("message should include minute token, got: %s", exitErr.Detail.Message)
	}
	if !strings.Contains(exitErr.Detail.Hint, "source_word") {
		t.Errorf("hint should mention source_word, got: %s", exitErr.Detail.Hint)
	}
}
...
func TestMinutesWordReplace_GenericInvalidParamsNotRewritten(t *testing.T) {
	...
	if exitErr.Detail != nil && exitErr.Detail.Type == "words_not_found" {
		t.Fatalf("generic 40001 must not be rewritten as words_not_found, got %#v", exitErr.Detail)
	}
}
