...
func ParseRoutes(specs []string) (*EventRouter, error) {
	...
	for _, spec := range specs {
		...
		if len(parts) != 2 {
			return nil, eventValidationError("--route", "invalid --route %q: expected format regex=dir:./path", spec)
		}
		pattern := parts[0]
		target := parts[1]
		...
		if err != nil {
			return nil, eventValidationErrorWithCause("--route", err, "invalid regex in --route %q: %s", spec, err)
		}

		if !strings.HasPrefix(target, "dir:") {
			return nil, eventValidationError("--route", "invalid --route target %q: must start with \"dir:\" prefix (format: regex=dir:./path)", target)
		}
		dir := strings.TrimPrefix(target, "dir:")
		if dir == "" {
			return nil, eventValidationError("--route", "invalid --route %q: directory path is empty", spec)
		}

		safeDir, err := validate.SafeOutputPath(dir)
		if err != nil {
			return nil, eventValidationErrorWithCause("--route", err, "invalid --route %q: %s", spec, err)
		}
