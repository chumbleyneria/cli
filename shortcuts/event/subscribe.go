...
var EventSubscribe = common.Shortcut{
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if outputDir != "" {
			...
			if err != nil {
				return eventValidationErrorWithCause("--output-dir", err, "unsafe --output-dir: %s", err)
			}
			outputDir = safePath
		}
		...
		if !forceFlag {
			...
			if err != nil {
				return eventFileIOError(err, "failed to create event subscriber lock: %s", err)
			}
			if err := lock.TryLock(); err != nil {
				if errors.Is(err, lockfile.ErrHeld) {
					...
					).WithParam("--force").WithHint("stop the existing subscriber for this app, or rerun with --force if you accept split event delivery").WithCause(err)
				}
				return eventFileIOError(err, "failed to acquire event subscriber lock: %s", err)
			}
			defer lock.Unlock()
		}
		...
		if err != nil {
			return eventValidationErrorWithCause("--filter", err, "invalid --filter regex %q: %s", filterStr, err)
		}
		...
		case err, ok := <-startErrCh:
			...
			if err != nil {
				return eventNetworkError(err, "WebSocket connection failed: %s", err)
			}
			return nil
		}
	},
